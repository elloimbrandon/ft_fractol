...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/10/03 16:22:23 by brfeltz           #+#    #+#             */
/*   Updated: 2019/10/03 16:29:36 by brfeltz          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_mandelbrot(t_info *info)
{
	...
	while (info->mandel->x != WIDTH)
	{
		...
		while (info->mandel->y != HEIGHT)
		{
			...
			(sqrt((info->mandel->imag_x * info->mandel->imag_x)
				+ (info->mandel->imag_y * info->mandel->imag_y)) < 3))
				square_root_mandel(info->mandel);
			if (info->mandel->iterate_max == info->mandel->count)
				all_pixel(info, info->mandel->x, info->mandel->y, 0);
			else
				all_pixel(info, info->mandel->x, info->mandel->y,
					info->mandel->count * info->mandel->color);
			info->mandel->y++;
		}
		info->mandel->x++;
	}
	...
	while (info->julia->x != WIDTH)
	{
		...
		while (info->julia->y != HEIGHT)
		{
			...
			while (++info->julia->count != info->julia->iterate_max &&
				info->julia->imag_x * info->julia->imag_x +
					info->julia->imag_y * info->julia->imag_y < 4)
				square_root_julia(info->julia);
			if (info->julia->iterate_max == info->julia->count)
				all_pixel(info, info->julia->x, info->julia->y, 0);
			else
				all_pixel(info, info->julia->x, info->julia->y,
					info->julia->count * info->julia->color);
			info->julia->y++;
		}
		info->julia->x++;
	}
	...
	while (info->burn_s->x != WIDTH)
	{
		...
		while (info->burn_s->y != HEIGHT)
		{
			...
			while (info->burn_s->count != info->burn_s->iterate_max &&
				info->burn_s->real_x * info->burn_s->real_x +
					info->burn_s->real_y * info->burn_s->real_y < 4)
				square_root_burn_s(info->burn_s);
			if (info->burn_s->iterate_max == info->burn_s->count)
				all_pixel(info, info->burn_s->x, info->burn_s->y, 0);
			else
				all_pixel(info, info->burn_s->x, info->burn_s->y,
					info->burn_s->count * info->burn_s->color);
			info->burn_s->y++;
		}
		info->burn_s->x++;
	}
